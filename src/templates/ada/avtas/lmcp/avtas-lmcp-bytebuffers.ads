...
package AVTAS.LMCP.ByteBuffers is
   ...
   with
     Pre'Class  =>
       ...
       --  number of message bytes remaining in the buffer. This ensure we
   ...
   procedure Put_String (This : in out ByteBuffer;  Value : String) with
     Pre'Class  =>
       --  Like the C++ version, we write lengths as a UInt16, so (unlike the
       --  C++ version) so we allow at most a length of UInt16'Last. The C++
       --  version truncates and prints an error msg at run-time for lengths
       --  > UInt16'Last. With the precondition we will get an exception,
       --  a more robust approach than losing message content somewhat
       --  silently.
       Value'Length <= UInt32 (UInt16'Last) and then
   ...
   procedure Put_Unbounded_String (This : in out ByteBuffer; Value : Unbounded_String) with
     Pre'Class  =>
       ...
       --  a more robust approach than losing message content somewhat
       --  silently.
       Length (Value) <= Integer (UInt16'Last) and then
   ...
   --
   --  NB: this routine doesn't write the length into the buffer because
